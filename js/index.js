    ...
    const lensCopy = {
      vision: {
        ...
        copy: "把检测、分割和多场景感知问题拆成数据、误差、实时性和可部署性几个可验证的环节。",
        meter: "82%"
      },
      edge: {
        title: "Edge Deployment",
        copy: "关注模型导出、量化、推理后端、相机链路和现场性能，让算法真的跑在设备上。",
        meter: "88%"
      },
      robot: {
        title: "Robot Learning",
        copy: "把视觉、触觉、机械臂控制和模仿学习数据合在一起，优先打通可复现实验链路。",
        meter: "74%"
      },
      loop: {
        title: "Engineering Loop",
        copy: "从现象定位到基准、日志、复现脚本和交付路径，尽量让每个问题都能被再次验证。",
        meter: "91%"
      }
    };
    ...
    function languageColor(language) {
      ...
      return `hsl(${Math.abs(hash) % 360} 64% 58%)`;
    }
    ...
    function setupResponsiveScale() {
      ...
      const scaledPixels = [
        ...
        860, 876, 880, 920, 1060, 1080, 1120, 1180, 1260, 1280, 1360, 1640, 1720, 1880, 1960, 1980
      ];
      ...
      const update = () => {
        ...
        const rawScale = isDesktop
          ? Math.min(window.innerWidth / baseWidth, window.innerHeight / baseHeight)
          : 1;
        const scale = isDesktop ? Math.min(1.5, Math.max(0.5, rawScale)) : 1;
    ...
    function setupReveal() {
      ...
      window.observeReveals = (nodes) => {
        nodes.forEach((node) => {
          if (!revealItems.has(node)) {
            revealItems.add(node);
            observer.observe(node);
          }
        });
      };
      window.observeReveals(document.querySelectorAll(".reveal"));
    }
    ...
    function setupPageSnap() {
      ...
      const getPages = () => Array.from(document.querySelectorAll(
        "#top > .hero, .hero-bridge, #profile, .life-bridge, #life, #projects, #repos, .contact-bridge, #contact"
      ))
        .sort((a, b) => (
    ...
    function setupConstellation() {
      ...
      function resize() {
        ...
        const count = Math.min(96, Math.max(42, Math.floor(width * height / 17000)));
        points = Array.from({ length: count }, (_, index) => ({
          x: Math.random() * width,
          y: Math.random() * height,
          vx: (Math.random() - 0.5) * 0.28,
          vy: (Math.random() - 0.5) * 0.28,
          size: index % 9 === 0 ? 2.2 : 1.3,
          hue: index % 7 === 0 ? "168, 239, 117" : index % 5 === 0 ? "232, 132, 122" : index % 3 === 0 ? "78, 156, 255" : "98, 231, 215"
        }));
      }
      ...
      function draw() {
        ...
        for (let i = 0; i < points.length; i += 1) {
          for (let j = i + 1; j < points.length; j += 1) {
            ...
            if (distance < 132) {
              ctx.strokeStyle = `rgba(98, 231, 215, ${0.18 * (1 - distance / 132)})`;
              ctx.lineWidth = 1;
              ctx.beginPath();
              ctx.moveTo(a.x, a.y);
              ctx.lineTo(b.x, b.y);
        ...
        for (const point of points) {
          ...
          const glow = pointerDistance < 180 ? 0.28 : 0.12;
          ctx.fillStyle = `rgba(${point.hue}, ${glow})`;
          ctx.beginPath();
    ...
    function setupLensConsole() {
      ...
      let activeLens = buttons.find((button) => button.classList.contains("is-active"))?.dataset.lens || "vision";
      ...
      const previewLens = (lens) => {
        if (!lens || !lensCopy[lens]) return;
        const isPreviewingActive = lens === activeLens;
        cards.forEach((card) => {
          card.classList.toggle("is-preview", card.dataset.lensCard === lens && !isPreviewingActive);
        });
      ...
      const activateLens = (lens) => {
        ...
        cards.forEach((card) => {
          const isActive = card.dataset.lensCard === lens;
          card.classList.toggle("is-active", isActive);
          card.classList.remove("is-preview", "is-activating");
          if (isActive) {
            void card.offsetWidth;
            card.classList.add("is-activating");
            window.setTimeout(() => card.classList.remove("is-activating"), 380);
          }
        });
        setDetailVisibility(activeLens);
      };
    ...
    function setupLifeArchive() {
      ...
      const finishDrag = () => {
        ...
        const slideWidth = stage.getBoundingClientRect().width || window.innerWidth || 1;
        const visiblePageRatio = dragDeltaX / slideWidth;
        if (visiblePageRatio <= -0.5) {
          setActiveLife(order[(activeIndex + 1) % order.length], {
            direction: "next"
          });
        } else if (visiblePageRatio >= 0.5) {
          setActiveLife(order[(activeIndex - 1 + order.length) % order.length], {
            direction: "prev"
          });
        } else if (Math.abs(dragDeltaX) > 0) {
          setActiveLife(order[activeIndex]);
        } else {
          setActiveLife(order[activeIndex], { instant: true });
        }
        dragStartX = 0;
        dragDeltaX = 0;
      };

      const snapLifeTrackToActive = () => {
        if (dragging || isWrappingLife) return;
        setActiveLife(order[activeIndex], { instant: true });
      };
      ...
      window.addEventListener("resize", () => {
        window.requestAnimationFrame(snapLifeTrackToActive);
      }, { passive: true });
    ...
    function setupProjectExplorer() {
      ...
      function visibleProjects() {
        return projectData.filter((project) => activeFilter === "All" || project.category === activeFilter);
      }
      ...
      function renderDetail(project) {
        detail.innerHTML = `
          ...
          `}
        `;
        detail.querySelectorAll("[data-private-project]").forEach((button) => {
      ...
      function renderProjects() {
        ...
        grid.innerHTML = projects.map((project, index) => `
          <article class="project-card tilt-card ${index === activeIndex ? "is-active" : ""}" data-project-index="${index}">
            <div>
        ...
        grid.querySelectorAll("[data-private-project]").forEach((button) => {
          button.addEventListener("click", () => showToast(`${button.dataset.privateProject} · Private project.`));
        });
    ...
    function setupRepositories() {
      ...
      function renderLanguages() {
        ...
        languageFilter.innerHTML = items.map((language) => `
          <button class="chip-button ${state.language === language ? "is-active" : ""}" type="button" data-language="${escapeHtml(language)}">
            ${language !== "All" ? `<span class="language-dot" style="--language-color: ${languageColor(language)}"></span>` : `<i data-lucide="layers-3"></i>`}
      ...
      function renderRepos() {
        ...
        const repoCards = repos.map((repo) => {
          ...
          return `
            <article class="repo-card reveal" style="--language-color: ${languageColor(language)};">
              <div>
                <h3>
              ...
              <div>
                <div class="repo-meta">
                  ...
                  <span><i data-lucide="clock-3"></i>${formatDate(repo.pushed_at)}</span>
                  ${repo.fork ? `<span><i data-lucide="git-pull-request"></i>Fork</span>` : ""}
                </div>
                <div class="repo-links">
                  ...
                  ${homepage ? `
                    <a class="repo-link" href="${homepage}" target="_blank" rel="noreferrer">
        ...
        grid.innerHTML = `
          ${repoCards}
          <div class="repo-signals reveal" aria-label="More repositories coming soon">
            <strong>Comming soon！</strong>
          </div>
        `;

        updateIcons();
        window.observeReveals?.(grid.querySelectorAll(".reveal"));
      }
